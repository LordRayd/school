...
public class AnalyseurTypes {
  ...
  class Synthesized{ // une valeur parmi ces quatre
    ...
    public  Synthesized parcours(Noeud node) {
       ...
       case ARGUMENT_LIST://TODO param multi
       case PARAMETER_DECLARATION_LIST: //TODO param multi
       ...
       case INTEGER_VALUE:
        ...
        case FUNCTION_CALL: { //TODO param multi
            ...
            List<TypeBasic> param_type_list= ((TypeFunction) proc_type).getParamTypeList();
            if (node.fils[1] != null) {
            	List<TypeBasic> arg_type_list = (List<TypeBasic>) parcours(node.fils[1]).type;
                if (param_type_list == null)
                    Erreur.compilation(Messages.TYPE_ARGS_NUMBER,node.ligne);
                Iterator<TypeBasic> iter_arg = arg_type_list.iterator();
                Iterator<TypeBasic> iter_param = param_type_list.iterator();
